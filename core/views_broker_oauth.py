...
def callback(request):
    ...
    websocket_balance = {}; websocket_health = 'not_checked'
    try:
        websocket_balance = asyncio.run(_verify_authenticated_websocket(access_token, account_id)); websocket_health = 'verified'
    except Exception as exc:
        websocket_health = 'degraded'; logger.warning('deriv_oauth_websocket_health_degraded', extra={'account_id': account_id, 'error': str(exc)})
    ...
    if request.user.is_authenticated:
        ...
        user = User.objects.create(username=f'deriv_{account_id}', first_name='Deriv'); user.set_unusable_password(); user.save(update_fields=['password'])
    ...
    broker_account.user = user; broker_account.currency = currency; broker_account.balance = balance_value
    broker_account.equity = websocket_balance.get('equity') if websocket_balance.get('equity') is not None else balance_value
    broker_account.status = 'active'; broker_account.is_preferred = True
    ...
    broker_account.set_access_token(access_token); broker_account.set_refresh_token(token_data.get('refresh_token', '')); broker_account.expires_at = expires_at; broker_account.token_status = 'active'; broker_account.last_refresh = timezone.now(); broker_account.last_synced_at = timezone.now(); broker_account.save()
    BrokerAccount.objects.filter(user=user).exclude(pk=broker_account.pk).update(is_preferred=False)

    # OAuth verification establishes an authenticated broker session. Persist
    # that state immediately so every API/UI consumer sees the same connection
    # truth before the first background sync runs.
    ...
    DerivOAuthService.clear_oauth_session(request)
    if websocket_health == 'degraded': messages.warning(request, f'Deriv account {account_id} connected. Live broker health is temporarily degraded; AlgoBot will retry automatically.')
    else: messages.success(request, f'Deriv account {account_id} connected successfully.')
