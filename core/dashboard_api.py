...
class DashboardViewSet(viewsets.ViewSet):
    ...
    def account_overview(self, request):
        account = (
            BrokerAccount.objects.filter(user=request.user, status="active", broker__status="active")
    ...
    def active_trades(self, request):
        rows = Order.objects.filter(user=request.user, status="executed").order_by("-created_at")[: self._limit(request)]
        return Response({"status": "success", "count": len(rows), "data": [
            {"id": row.id, "symbol": row.symbol, "stake": row.stake, "strategy": row.strategy, "created_at": row.created_at}
            for row in rows
        ]})
    ...
    def trade_history(self, request):
        ...
        return Response({"status": "success", "total": len(rows), "count": len(rows), "data": [
            {"id": row.id, "symbol": row.symbol, "stake": row.stake, "direction": row.direction, "status": row.status, "strategy": row.strategy, "created_at": row.created_at, "broker_reference": row.broker_reference}
            for row in rows
        ]})
