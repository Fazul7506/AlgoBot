...
def _preferred_deriv_account(user):
    return (BrokerAccount.objects.filter(user=user, broker__broker_type='deriv').select_related('broker').order_by('-is_preferred', '-last_synced_at', '-id').first())
...
def deriv_login(request):
    ...
    try:
        ...
        return HttpResponse('AlgoBot could not start the secure broker connection. Please try again in a moment.', status=503)
...
def broker_marketplace_page(request):
    return render(request, 'core/broker_marketplace.html', {'brokers': Broker.objects.filter(status='active').order_by('name')})

@login_required
def strategy_builder_page(request):
    return render(request, 'core/strategy_builder.html')

@login_required
def operations_module_page(request, module):
    modules = {
        'automation': {'title':'Automation','eyebrow':'WORKFLOW OPERATIONS','description':'Review workflows, execution history and approval controls from the API.','actions':[('Open workflows','/automation/','link')],'endpoints':[('/api/automation/workflows/','Workflows','GET'),('/api/automation/history/','Execution history','GET')]},
        'notifications': {'title':'Notifications','eyebrow':'DELIVERY OPERATIONS','description':'Inspect notification preferences, templates and recent delivery records.','actions':[('Refresh','/operations/notifications/','link')],'endpoints':[('/api/notifications/','Notifications','GET'),('/api/notifications/delivery/','Delivery records','GET')]},
        'brokers': {'title':'Broker accounts','eyebrow':'BROKER OPERATIONS','description':'Manage the connected broker accounts that supply the workspace with trading data.','actions':[('Connect broker','/brokers/connect/','link'),('Browse brokers','/brokers/marketplace/','link')],'endpoints':[('/api/brokers/accounts/','Connected accounts','GET'),('/api/broker-health/','Broker health','GET')]},
    }
