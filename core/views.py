...
def _preferred_deriv_account(user):
    return BrokerAccount.objects.filter(user=user, broker__broker_type="deriv").select_related("broker").order_by("-last_synced_at", "-id").first()
...
def _auth_gateway(request, error=None):
    return render(request, "core/auth_gateway.html", {"auth_gateway_error": error})
...
def deriv_login(request):
    ...
    if not is_valid:
        ...
        return _auth_gateway(request, "Broker authentication is not configured on this deployment yet. Platform administrators must complete the Deriv OAuth configuration before sign-in can proceed.")
    ...
    try:
        ...
        logger.exception("deriv_oauth_start_failed")
        return _auth_gateway(request, "AlgoBot could not start the secure broker connection. Please try again in a moment.")
