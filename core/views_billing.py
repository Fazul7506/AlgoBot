...
def billing_checkout(request):
    ...
    if not result.get("url"):
        invoice.metadata = {**invoice.metadata, "state": "checkout_failed", "error": result.get("error", "Unable to create checkout")}
    ...
    invoice.metadata = {**invoice.metadata, "state": "checkout_open", "reference": result.get("reference"), "tracking_id": result.get("order_tracking_id"), "session_id": result.get("session_id")}
...
def billing_reconcile(request):
    ...
    if provider == service.PESAPAL:
        ...
        paid = state in {"COMPLETED", "COMPLETED_SUCCESS", "SUCCESS"}
    ...
    if paid:
        ...
        Payment.objects.filter(invoice=invoice).update(status="complete")
    elif state in {"FAILED", "CANCELLED", "INVALID"}:
        Payment.objects.filter(invoice=invoice).update(status=state.lower())
