"""Authenticated billing API: plan discovery, hosted checkout and reconciliation."""
...
class RequestBoundPaymentService(PaymentService):
    """Use the public HTTPS host that the customer is actually visiting.

    This prevents a stale/local BASE_URL from producing an invalid hosted
    checkout redirect such as a localhost URL in production.
    """
...
def _create_checkout(request):
    ...
    if plan["plan"] == "FREE": return Response({"detail":"Use the plan change endpoint to switch to FREE."}, status=status.HTTP_400_BAD_REQUEST)
    if not plan["configured"]: return Response({"detail":f"{plan['plan']} is not configured for checkout yet."}, status=status.HTTP_503_SERVICE_UNAVAILABLE)
    ...
    result = RequestBoundPaymentService(request).create_checkout_session(request.user, CheckoutPlan(**{k:plan[k] for k in ("plan","price_cents","currency","recurring")}), provider=provider)
    if not result.get("url"):
        invoice.metadata={**(invoice.metadata or {}),"state":"checkout_failed","error":result.get("error","Unable to create checkout")}; invoice.save(update_fields=["metadata"])
...
@api_view(["POST"])
@permission_classes([IsAuthenticated])
def billing_checkout(request): return _create_checkout(request)
...
def billing_reconcile(request):
    ...
    if provider==service.PESAPAL:
        tracking=(invoice.metadata or {}).get("tracking_id") or invoice.external_id; result=service.get_pesapal_transaction_status(str(tracking)) if tracking else None; state=str((result or {}).get("payment_status_description","")).upper(); paid=state in {"COMPLETED","COMPLETED_SUCCESS","SUCCESS","SUCCEEDED","PAID"}
    elif provider==service.INTASEND:
        result=service.get_intasend_payment_status(invoice.external_id) if invoice.external_id else None; state=str((result or {}).get("state","")).upper(); paid=state in {"COMPLETE","COMPLETED","SUCCESS","SUCCEEDED","PAID"}
    else: return Response({"detail":"Unsupported payment provider."},status=status.HTTP_400_BAD_REQUEST)
    if paid: _activate(invoice,plan); Payment.objects.filter(invoice=invoice).update(status="COMPLETED")
    elif state in {"FAILED","CANCELLED","CANCELED","INVALID","REVERSED"}: Payment.objects.filter(invoice=invoice).update(status="FAILED")
