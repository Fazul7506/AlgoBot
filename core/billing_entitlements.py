...
FEATURE_LABELS = {"api_calls":"Execution API calls", "strategies":"active strategy configurations", "backtests":"backtests", "predictions":"AI predictions", "orders":"orders", "broker_accounts":"connected brokers", "automations":"automation runs", "live_orders":"live-money orders"}
...
def usage(user, metric, window="day"):
    ...
    if metric == "strategies":
        try:
            ...
            return StrategyConfiguration.objects.filter(user=user, is_active=True).count()
        except Exception: return 0
    return 0
...
def entitlement_payload(user):
    ...
    if subscription is None: return {"plan":plan.key,"name":plan.name,"active":False,"expires_at":None,"recurring":False,"priority":plan.priority,"support":plan.support,"features":{"live_trading":plan.live_trading,"advanced_ai":plan.advanced_ai},"usage":{},"reset_at":reset_at("day").isoformat()}
    items = {}
