...
(() => {
  ...
  function bindSidebarScrollState(sidebar, scrollHost = sidebar.querySelector('nav')) {
    ...
    const applySavedPosition=saved=>{if(!saved)return false;const maxScroll=Math.max(0,sidebar.scrollHeight-sidebar.clientHeight);const target=saved.atBottom?maxScroll:Math.min(saved.top,maxScroll);scrollHost.scrollTop=target;lastKnownTop=target;return scrollHost.scrollTop===target;};
    ...
    const recordScroll=()=>{lastKnownTop=Math.max(0,sidebar.scrollTop||0);savePosition();}; scrollHost.addEventListener('scroll',recordScroll,{passive:true});
  ...
  function bindNavigation() {
    ...
    syncActiveNavigation({anchor:true}); bindSidebarScrollState(sidebar, nav);
