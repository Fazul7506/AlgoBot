...
(() => {
  ...
  function bindNavigation() {
    ...
    const setMobileOpen=open=>{const next=!!open;sidebar.classList.toggle('is-open',next);if(backdrop)backdrop.hidden=!next;if(mobile){mobile.setAttribute('aria-expanded',String(next));mobile.setAttribute('aria-label',next?'Close navigation':'Open navigation');const icon=mobile.querySelector('.material-symbols-rounded');if(icon)icon.textContent=next?'close':'menu';}document.body.classList.remove('mobile-nav-open');document.documentElement.classList.remove('mobile-nav-open');};
    setMobileOpen(false);
