...
(() => {
  ...
  function bindNavigation() {
    ...
    if(toggle){const setCollapsed=collapsed=>{if(window.innerWidth<=1100)return;sidebar.classList.toggle('is-collapsed',!!collapsed);shell?.classList.toggle('sidebar-collapsed',!!collapsed);document.body.classList.toggle('sidebar-collapsed',!!collapsed);toggle.setAttribute('aria-expanded',String(!collapsed));toggle.setAttribute('aria-label',collapsed?'Expand navigation':'Collapse navigation');toggle.setAttribute('title',collapsed?'Expand navigation':'Collapse navigation');const icon=toggle.querySelector('.material-symbols-rounded');if(icon)icon.textContent=collapsed?'left_panel_open':'left_panel_close';sidebar.querySelectorAll('nav a,.sidebar-new-trade').forEach(link=>{const label=(link.querySelector('span:not(.material-symbols-rounded)')?.textContent||link.getAttribute('aria-label')||'').trim();if(label){link.setAttribute('aria-label',label);link.setAttribute('title',collapsed?label:'');}});try{localStorage.setItem(storageKey,collapsed?'1':'0');}catch(_){} };try{if(window.innerWidth>1100&&localStorage.getItem(storageKey)==='1')setCollapsed(true);}catch(_){}toggle.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();setCollapsed(!sidebar.classList.contains('is-collapsed'));syncActiveNavigation({anchor:false});});}
    
    document.addEventListener('keydown',event=>{if(event.key==='Escape')setMobileOpen(false);}); window.addEventListener('resize',()=>{if(window.innerWidth>1100)setMobileOpen(false);syncActiveNavigation({anchor:false});});
  }
