(() => {
  ...
  const money = (value, currency = 'USD') => value == null || value === '' || Number.isNaN(Number(value)) ? '—' : (typeof window.AlgoBotMoney?.format === 'function' ? window.AlgoBotMoney.format(value, currency) : `${String(currency || 'USD').toUpperCase() === 'USD' ? '
 ...
 : String(currency || '').toUpperCase() + ' '}${Number(value).toLocaleString(undefined, {minimumFractionDigits:2, maximumFractionDigits:8})}`);
  const pct = value => value == null || Number.isNaN(Number(value)) ? '—' : `${Number(value).toFixed(1)}%`;
  const label = value => String(value ?? '').replaceAll('_', ' ').replace(/\b\w/g, c => c.toUpperCase()) || '—';
  const tone = value => { const v = String(value || '').toUpperCase(); return v === 'BUY' ? 'buy' : v === 'SELL' ? 'sell' : 'hold'; };
  const stateText = value => String(value || 'WAITING').replaceAll('_', ' ');
