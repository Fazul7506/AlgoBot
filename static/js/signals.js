(() => {
  ...
  function focus(row) {
    if (!row) {
      ...
      if ($('focusConfidence')) $('focusConfidence').textContent='0%'; if ($('focusConfidenceBar')) $('focusConfidenceBar').style.width='0%'; if ($('focusEvidence')) $('focusEvidence').innerHTML='<span class="muted">No signal selected.</span>'; renderSpec(null); return;
    }
    ...
    $('focusConfidence').textContent=pct(row.confidence??0); $('focusConfidenceBar').style.width=`${Math.max(0,Math.min(100,Number(row.confidence)||0))}%`; $('focusBaseline').textContent=row.baseline_direction?`${row.baseline_direction} · ${pct(row.baseline_confidence)}`:'No Analysis baseline';
