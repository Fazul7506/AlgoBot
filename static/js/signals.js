(() => {
  ...
  function render(){
    ...
    if(grid) grid.innerHTML=filtered.slice(0,12).map(r=>{const c=Math.max(0,Math.min(100,Number(r.confidence||0)));return `<article class="signal-card"><div class="signal-card-head"><div><div class="signal-symbol">${esc(r.symbol)}</div><small>${esc(r.strategy||'Strategy signal')}</small></div><strong class="signal-direction">${esc(r.direction||'NEUTRAL')}</strong></div><div class="signal-meta"><div><span>Confidence</span><strong class="signal-confidence">${pct(r.confidence)}</strong></div><div><span>Regime</span><strong>${esc(r.market_regime||'—')}</strong></div><div><span>Entry</span><strong>${esc(r.entry_price??'—')}</strong></div><div><span>Stop / Target</span><strong>${esc(r.stop_loss??'—')} / ${esc(r.take_profit??'—')}</strong></div></div><div class="signal-bar" aria-label="Confidence ${c}%"><span style="width:${c}%"></span></div><div class="signal-actions"><a class="btn" href="/trading/">Open terminal</a>${r.was_executed?'<span class="btn">Executed</span>':''}</div></article>`}).join('')||'<div class="panel"><p>No signals match the current search.</p></div>';
