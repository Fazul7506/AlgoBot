(() => {
  ...
  grid.addEventListener('click', async e=>{const b=e.target.closest('button');if(!b)return;try{b.disabled=true; if(b.dataset.sync){const result=await api(`/api/brokers/accounts/${b.dataset.sync}/sync/`,{method:'POST'});if(result?.account)window.dispatchEvent(new CustomEvent('algobot:account-synced',{detail:result.account}));} else if(b.dataset.select){const accountId=b.dataset.select;const account=accounts.find(a=>String(a.id)===String(accountId));const accountType=['demo','real'].includes(String(account?.account_type||'').toLowerCase())?String(account.account_type).toLowerCase():'';const result=await api(`/api/brokers/accounts/${accountId}/select/`,{method:'POST',body:JSON.stringify(accountType?{account_type:accountType}:{})});const selected=result?.active_account||result?.account;if(!selected?.id)throw new Error('Broker did not confirm the selected account.');window.dispatchEvent(new CustomEvent('algobot:account-changed',{detail:selected}));} else if(b.dataset.connect) await api('/api/brokers/connect/',{method:'POST',body:JSON.stringify({broker_id:b.dataset.broker,account_id:b.dataset.connect})}); else if(b.dataset.disconnect) await api('/api/brokers/disconnect/',{method:'POST',body:JSON.stringify({broker_id:b.dataset.broker,account_id:b.dataset.disconnect})}); await load();}catch(err){alert(err.message);}finally{b.disabled=false;}});
