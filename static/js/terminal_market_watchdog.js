...
(() => {
  ...
  const connect = symbol => {
    ...
    ws = new WebSocket(WS);
    ws.onopen = () => {
      try { ws.send(JSON.stringify({ticks:symbol,subscribe:1,req_id:Date.now()})); } catch (_) { close(); }
    };
    ws.onmessage = e => {
      try {
        const d = JSON.parse(e.data);
        if (d.msg_type === 'tick' && !d.error && d.tick?.quote != null) updateQuote(d.tick.quote);
      } catch (_) {}
    };
    ws.onerror = () => { close(); };
  ...
  const selectedSymbol = () => {
    const el = $('#symbol');
    return String(el?.value || el?.dataset?.symbol || '').trim();
  };
  ...
  function boot() {
    ...
    timer = window.setInterval(() => {
      ...
      if (!symbol) return;
      if (symbol !== lastSymbol) { close(); lastQuoteMutation = Date.now(); }
      if (!active && Date.now() - lastQuoteMutation >= STALE_MS) connect(symbol);
    }, 1000);
