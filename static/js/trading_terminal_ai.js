...
(() => {
  ...
  async function analyse(){
    if(analysing)return;const button=$('[data-ai-analyze]'),symbol=$('#symbol')?.value;if(!symbol){show('Select a broker instrument before running AI analysis.');return;}
    ...
    try{const data=await api('/api/ai/predict/',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({symbol,timeframe:'M1'}),notifyOnError:false},30000);render(data);window.dispatchEvent(new CustomEvent('algobot:ai-analysis-updated',{detail:data}));}
  ...
  function boot(){if(!$('.terminal-page'))return;$('[data-ai-analyze]')?.addEventListener('click',()=>analyse());window.addEventListener('algobot:market-symbol-changed',resetForSymbol);window.addEventListener('algobot:broker-contract-selected',()=>{if(!analysing)show('Broker contract ready. Run Analyse market when needed.')});}
