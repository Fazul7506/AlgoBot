...
(() => {
  ...
  let analysing=false,scheduled=null,selectedTimeframe='M1';
  function show(message){text('[data-ai-explanation]',message)}
  function normalizeTimeframe(detail={}){const label=String(detail.label||'').trim().toLowerCase(),seconds=Number(detail.seconds);if(label){const match=label.match(/^(\d+)(s|m|h|d)$/);if(match){const n=Number(match[1]),unit=match[2].toUpperCase();return unit==='D'?`D${n}`:unit==='H'?`H${n}`:unit==='M'?`M${n}`:`S${n}`}}if(Number.isFinite(seconds)&&seconds>0){if(seconds%86400===0)return `D${seconds/86400}`;if(seconds%3600===0)return `H${seconds/3600}`;if(seconds%60===0)return `M${seconds/60}`;return `S${seconds}`}return selectedTimeframe;}
  function setTimeframe(detail){selectedTimeframe=normalizeTimeframe(detail);window.__algobotTerminalTimeframe=selectedTimeframe;resetForSymbol();}
