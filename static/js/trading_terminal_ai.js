...
(() => {
  ...
  const selectedAccount=()=>window.AlgoBotAccountContext?.getSelected?.()||window.AlgoBotBrokerState?.get?.()?.account||null;
  let analysing=false;
  ...
  async function analyse(){
    if(analysing)return;const button=$('[data-ai-analyze]'),symbol=$('#symbol')?.value,account=selectedAccount();if(!symbol){show('Select a broker instrument before running AI analysis.');return}if(!account?.id){show('Select a connected broker account before running AI analysis.');return}
    ...
    try{const data=await api('/api/ai/predict/',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({symbol,timeframe:'M1',account_id:account.id}),notifyOnError:false},30000);render(data);window.dispatchEvent(new CustomEvent('algobot:ai-analysis-updated',{detail:data}))}
  ...
  function resetForSymbol(){text('[data-ai-prediction]','Not analysed');text('[data-ai-recommendation]','Not analysed');text('[data-ai-confidence-card]','Not analysed');text('[data-ai-confidence]','Not analysed');text('[data-ai-regime]','Not analysed');show('Choose Analyse market when you want a fresh AI decision. AI analysis never places an order.');window.__algobotAiOrderContext=null;window.dispatchEvent(new CustomEvent('algobot:ai-gate-updated',{detail:{actionable:false,reset:true}}))}
  function boot(){if(!$('.terminal-page'))return;$('[data-ai-analyze]')?.addEventListener('click',()=>analyse());window.addEventListener('algobot:market-symbol-changed',resetForSymbol);window.addEventListener('algobot:broker-contract-selected',()=>{if(!analysing)show('Broker contract ready. Run Analyse market when needed.')});window.addEventListener('algobot:account-changed',resetForSymbol)}
