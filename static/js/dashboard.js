(() => {
  ...
  async function load() {
    ...
    try {
      ...
      const signals=selectedSymbol ? await window.AlgoBotFrontendData.request(`/api/dashboard/signals/?limit=8&symbol=${encodeURIComponent(selectedSymbol)}`).catch(()=>null) : null;
