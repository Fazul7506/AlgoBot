(() => {
  ...
  async function bootstrapBrokerState() {
    if (brokerBootstrapped) return window.AlgoBotBrokerState?.get();
    brokerBootstrapped = true;
    try {
      const payload = await request('/api/brokers/accounts/');
      const accounts = list(payload).filter(Boolean);
      if (!accounts.length) {
        window.AlgoBotBrokerState?.transition('NO_BROKER', {account:null, connection:null, balances:null}, 'accounts-empty');
        return window.AlgoBotBrokerState?.get();
      }
      let account = accounts.find(a => a.is_preferred) || accounts.find(a => ['active','connected','ready','online','synchronized','syncing'].includes(String(a.status || '').toLowerCase())) || accounts[0];
      // The account endpoint is persisted state; force a broker confirmation before
      // presenting its balance/equity as live data.
      window.AlgoBotBrokerState?.transition('CONNECTING', {account, connection:null}, 'broker-account-discovered');
      const sync = await request(`/api/brokers/accounts/${account.id}/sync/`, {method:'POST', headers:{'Content-Type':'application/json'}, body:'{}'});
      const liveAccount = sync?.account || account;
      window.AlgoBotBrokerState?.setAccount(liveAccount, 'broker-account-synchronized');
      return window.AlgoBotBrokerState?.get();
    } catch (error) {
      brokerBootstrapped = false;
      window.AlgoBotBrokerState?.transition('DEGRADED', {lastError:error?.message || 'Unable to verify broker account'}, 'broker-bootstrap-failed');
      return window.AlgoBotBrokerState?.get();
    }
  }
  ...
  async function load() {
    ...
    let state = window.AlgoBotBrokerState?.get();
    if (!connected()) state = await bootstrapBrokerState();
    renderAccount(state);
    ...
    try {
      ...
      setHtml('[data-dashboard-brokers]',account?`<span><b></b>${esc(account.broker?.name||account.broker_name||'Broker')} · ${esc(account.broker_account_id||account.account_id)} · ${esc(window.AlgoBotBrokerState?.get()?.status||'READY')}</span>`:empty('No connected broker account'));
  ...
  function boot() {
    ...
    $('[data-dashboard-refresh]')?.addEventListener('click',()=>{ brokerBootstrapped=false; load(); });
