(() => {
  ...
  async function ensureBrokerState(){if(typeof window.AlgoBotBrokerSync==='function'){try{await window.AlgoBotBrokerSync()}catch(_){}}return state()}
  function renderAccount(s){
    const a=s?.account;
    if(!a){['balance','equity','available','pnl'].forEach(k=>setText(`[data-kpi="${k}"]`,'Unavailable'));setText('[data-kpi-state="balance"]',s?.lastError||'No broker account returned by the backend');setText('[data-kpi-state="equity"]','Waiting for broker-backed account data');return}
  ...
  function renderBroker(s){const a=s?.account;setHtml('[data-dashboard-brokers]',a?`<span><b></b>${esc(a.broker?.name||a.broker_name||'Deriv')} · ${esc(a.broker_account_id||a.account_id||a.loginid||'')} · ${esc(s?.status||'READY')}</span>`:empty(s?.lastError||'No broker account returned by the backend.'))}
  async function load(){
    ...
    try{
      ...
      const results=await Promise.allSettled([request()('/api/positions/open/'),request()('/api/orders/'),request()('/api/market/snapshots/all_snapshots/'),request()('/api/dashboard/signals/?limit=8')]);
