(() => {
  ...
  const money = (value, currency = 'USD') => {
    if (value == null || value === '' || Number.isNaN(Number(value))) return 'Unavailable';
    if (typeof window.AlgoBotMoney?.format === 'function') return window.AlgoBotMoney.format(value, currency);
  ...
  function renderAccount(account, message = '') {
    ...
    setText('[data-kpi="balance"]', money(account.balance, currency).trim());
    setText('[data-kpi="equity"]', money(equity, currency).trim());
    setText('[data-kpi="available"]', money(account.free_margin ?? account.available_margin ?? account.available, currency).trim());
    setText('[data-kpi="pnl"]', pnl == null ? 'Unavailable' : money(pnl, currency).trim());
 ...
 : (code ? code + ' ' : '');
    return prefix + Number(value).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 8});
  };
  const setText = (selector, value) => { const node = $(selector); if (node) node.textContent = value; };
  const setHtml = (selector, value) => { const node = $(selector); if (node) node.innerHTML = value; };
  const empty = (message) => `<div class="empty-state">${esc(message)}</div>`;
