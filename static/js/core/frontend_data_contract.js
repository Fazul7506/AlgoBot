...
(() => {
  ...
  const resolveUrl=url=>{
    ...
    if(/^https?:\/\//i.test(raw)){
      ...
      if(productionApiBase&&['algobot.dpdns.org','www.algobot.dpdns.org'].includes(absolute.hostname))return `${productionApiBase}${absolute.pathname}${absolute.search}${absolute.hash}`;
      ...
      try{
        try{result=await fetchOnce(url,options,controller)}catch(error){
          ...
          if(!result){
            ...
            e.code=firstError?.name==='AbortError'?'API_TIMEOUT':'NETWORK_ERROR';e.status=0;e.retryable=safeMethods.has(method);
