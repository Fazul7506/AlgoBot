...
(() => {
  ...
  const isCloudflareChallenge = (response, text) => {
    ...
    return [403,429,503,520,521,522,524].includes(response.status) && (body.includes('just a moment') || body.includes('challenge-platform') || body.includes('cf-chl-') || body.includes('enable javascript and cookies to continue'));
  };
  ...
  async function request(url, options = {}, timeout = 25000) {
    ...
    const promise = (async () => {
      try {
        let result = await fetchOnce(url, options, controller);
        if (!result.response.ok && isCloudflareChallenge(result.response, result.text)) {
          const alias = cloudflareAlias(url);
          if (alias) result = await fetchOnce(alias, options, controller);
        }
        const { response, text } = result;
        let payload = {};
        try { payload = text ? JSON.parse(text) : {}; } catch (_) { payload = { detail:text }; }
        if (!response.ok) { const error = new Error(payload.detail || payload.message || (isCloudflareChallenge(response,text) ? 'Broker/backend request was blocked by the edge security challenge.' : `Request failed (${response.status})`)); error.status=response.status; throw error; }
