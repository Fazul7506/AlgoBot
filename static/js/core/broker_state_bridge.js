 ...
 * reconciles selected browser state with authoritative backend/broker state.
 */
(() => {
  'use strict';
  ...
  async function refreshAccounts(store){
    ...
    const contextId=window.AlgoBotAccountContext?.getSelectedId?.();
    const preferred=requestedAccountId||contextId;
    const requested=preferred&&accounts.find(a=>String(a.id)===String(preferred));
    const account=requested||accounts.find(a=>a.is_preferred||a.is_default||a.is_active)||accounts[0]||null;
    ...
    if(window.AlgoBotAccountContext?.getSelectedId?.()!==id)window.AlgoBotAccountContext?.load?.().catch(()=>{});
  ...
  async function verifyWithBroker(store,account){
    ...
    try{
      const payload=await requestJson('/api/brokers/connect/',{method:'POST',headers:{'Content-Type':'application/json','X-CSRFToken':csrf()},body:JSON.stringify({broker_id:account.broker.id,account_id:account.id})},CONNECT_TIMEOUT_MS);
      const verified=payload?.account;if(!verified)throw new Error('Backend broker connection returned no account payload.');
      store.setAccount(verified,'broker-live-connected');requestedAccountId=verified.id;
      window.dispatchEvent(new CustomEvent('algobot:account-synced',{detail:verified}));return verified;
    }catch(error){
      const current=store.get(),bs=error.payload?.broker_status;
      const state=(error.status===401||bs==='credentials_expired')?store.STATES.ERROR:(error.status===409?store.STATES.DISCONNECTED:(error.status===503||error.status===504||bs==='unavailable'||bs==='sync_timeout'||error.code==='BROKER_SYNC_TIMEOUT'?store.STATES.DEGRADED:store.STATES.ERROR));
  ...
  function syncFromBackend(accountId=null){
    ...
    if(accountId!=null)requestedAccountId=String(accountId);
    if(syncPromise)return syncPromise;
    syncPromise=(async()=>{try{const account=await refreshAccounts(store);return account?await verifyWithBroker(store,account):null}catch(error){const current=store.get();store.transition(current.account?store.STATES.DEGRADED:store.STATES.ERROR,{account:current.account,lastError:error.message},'backend-broker-account-error');window.dispatchEvent(new CustomEvent('algobot:account-context-error',{detail:error}));return current.account||null}finally{syncPromise=null}})();
    return syncPromise;
  }

  function schedule(){
    if(timer)clearInterval(timer);
    const start=()=>{if(!window.AlgoBotBrokerState){setTimeout(start,250);return}syncFromBackend();timer=setInterval(()=>syncFromBackend(window.AlgoBotAccountContext?.getSelectedId?.()||null),RETRY_INTERVAL_MS);setTimeout(()=>{if(timer)clearInterval(timer);timer=setInterval(()=>syncFromBackend(window.AlgoBotAccountContext?.getSelectedId?.()||null),HEALTH_REFRESH_MS)},RETRY_INTERVAL_MS*3)};
    start();
  }
  ...
  window.addEventListener('algobot:account-changed',event=>{
    ...
    if(id!=null){requestedAccountId=String(id);lastAccountId=String(id);window.AlgoBotBrokerState?.setAccount(event.detail,'account-changed');window.AlgoBotFrontendData?.cached?.('/api/brokers/accounts/');}
    syncFromBackend(id).catch(()=>{});
  });
  window.addEventListener('algobot:account-synced',event=>{
    if(!event.detail)return;
    requestedAccountId=String(event.detail.id);lastAccountId=String(event.detail.id);window.AlgoBotBrokerState?.setAccount(event.detail,'account-synced');
    knownAccounts=knownAccounts.map(a=>String(a.id)===String(event.detail.id)?event.detail:a);window.AlgoBotBrokerAccounts=knownAccounts.slice();
  });
