...
(() => {
  ...
  async function syncFromBackend() {
    ...
    busy = true;
    try {
      const response = await fetch('/api/brokers/accounts/', {credentials:'same-origin', headers:{Accept:'application/json'}, cache:'no-store'});
      ...
      if (!account) {
        ...
        lastAccountId = null; lastConnection = false; lastBalance = null; return;
      }
      ...
      /* Preserve known-good state during transient polling failures. */
  ...
  function schedule() {
    ...
    timer = setInterval(syncFromBackend, 10000);
  }
