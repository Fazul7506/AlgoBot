...
def market_intelligence(request):
    ...
    for market in symbols[:_limit(request)]:
        ...
        if snapshot is not None:
            ...
            if change > 0:
                score += 1; evidence.append('positive_price_change')
            elif change < 0:
                score -= 1; evidence.append('negative_price_change')
        ...
        if directions:
            ...
            if count > opposing:
                score += Decimal('1') if dominant == 'BUY' else Decimal('-1')
            evidence.append(f'signal_confluence_{dominant.lower()}')
        results.append({'symbol':market.symbol,'display_name':market.display_name,'market':market.market,'sub_market':market.sub_market,'status':'ready' if snapshot is not None else 'no_data','confluence_score':float(score),'signal_count':len(recent),'buy_signals':directions.get('BUY',0),'sell_signals':directions.get('SELL',0),'evidence':evidence,'snapshot_timestamp':snapshot.timestamp.isoformat() if snapshot else None,'source':'broker_snapshot_store_and_strategy_signals'})
...
def signal_lifecycle(request):
    """Expose signal lifecycle state derived from persisted signals."""
    ...
    for signal in qs[:_limit(request, 100, 100)]:
        ...
        lifecycle = 'executed' if signal.was_executed else ('active' if age_seconds <= 300 else 'expired')
