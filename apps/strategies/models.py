...
class Strategy(models.Model):
    ...
    class Meta:
        ordering = ['name']
        indexes = [models.Index(fields=['enabled', 'category']), models.Index(fields=['slug', 'version'])]
...
class StrategyExecution(models.Model):
    ...
    class Meta:
        ordering = ['-started_at']
        indexes = [models.Index(fields=['strategy', 'status']), models.Index(fields=['symbol', 'timeframe', '-started_at'])]
...
class StrategyPerformance(models.Model):
    ...
    net_profit = models.DecimalField(max_digits=18, decimal_places=8, default=0)
    drawdown = models.DecimalField(max_digits=12, decimal_places=4, default=0)
    sharpe_ratio = models.DecimalField(max_digits=12, decimal_places=4, default=0)
...
class StrategySignal(models.Model):
    ...
    symbol = models.CharField(max_length=40, db_index=True)
    signal = models.CharField(max_length=32, choices=[(x, x) for x in c.SIGNAL_TYPES])
    confidence = models.DecimalField(max_digits=5, decimal_places=2, default=0)
    ...
    class Meta:
        ordering = ['-timestamp']
        indexes = [models.Index(fields=['strategy', 'symbol', '-timestamp'])]
