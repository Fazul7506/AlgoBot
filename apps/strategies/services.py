...
class LiveMarketContextService:
    """Fetch broker candles and build a model-compatible live context."""
    def _run(self, awaitable):
        return asyncio.run(awaitable)

    def _granularity(self, timeframe):
        return {'M1': 60, 'M5': 300, 'M15': 900, 'M30': 1800, 'H1': 3600, 'H4': 14400, 'D1': 86400}.get(str(timeframe).upper(), 60)
    ...
    def build(self, config):
        ...
        if account is None or getattr(account, 'status', None) != 'active':
            raise RuntimeError('An active broker account is required for autonomous strategy execution')
        ...
        for item in items:
            try:
                candles.append({'open': float(item['open']), 'high': float(item['high']), 'low': float(item['low']), 'close': float(item['close']), 'volume': float(item.get('volume', 0) or 0), 'epoch': int(item.get('epoch', 0) or 0)})
            except (KeyError, TypeError, ValueError):
                continue
        if len(candles) < 25:
            raise RuntimeError(f'Insufficient live broker candle history for {config.symbol} {config.timeframe}: {len(candles)} usable candles')
        from trading.ai.features.simple_indicators import compute_basic_features
        rows = compute_basic_features(candles)
        latest, current = rows[-1], candles[-1]
...
class StrategyService:
    def sync_catalog(self):
        return [StrategyRepository().create_or_update_catalog(cls) for cls in registry.all().values()]
...
class StrategyExecutionService:
    def run_configuration(self, config, market_data=None, indicator_data=None):
        ...
        try:
            if market_data is None or indicator_data is None:
                market_data, indicator_data, handoff = LiveMarketContextService().build(config)
            else:
                handoff = {'source': 'caller_supplied', 'timeframe': config.timeframe}
            if not market_data or market_data.get('close') is None:
                raise RuntimeError('No live market price was supplied to strategy execution')

            cls = registry.get(config.strategy.slug)
            strat = cls(config, market_data, indicator_data)
            strat.initialize()
            strat.validate()
            ...
            if ai_enabled and criteria_passed:
                try:
                    ...
                    log.warning('AI enhancement unavailable; preserving strategy result', extra={'strategy': config.strategy.slug, 'error': ai_error})
            ...
            execution.latency_ms = (time.perf_counter() - start) * 1000
            execution.save()
            log.exception('Strategy execution failed')
            return execution


class StrategyPerformanceService:
    def recalculate(self, strategy):
        perf = StrategyPerformanceRepository().for_strategy(strategy)
        qs = strategy.executions.filter(status='completed').exclude(signal='HOLD')
        perf.total_trades = qs.count()
        perf.wins = strategy.executions.filter(context__outcome='win').count()
        perf.losses = strategy.executions.filter(context__outcome='loss').count()
        perf.win_rate = (perf.wins / perf.total_trades * 100) if perf.total_trades else 0
        perf.last_updated = timezone.now()
        perf.save()
        return perf
