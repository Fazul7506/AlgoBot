...
class BrokerConnectionContractTests(APITestCase):
    def setUp(self):
        ...
        self.account = BrokerAccount.objects.create(user=self.user, broker=self.broker, account_id="BROKER-ACCOUNT", status="active", credentials={})
    ...
    def test_switch_rejects_unconnected_unconfirmed_account(self):
        response = self.client.post(reverse("broker-accounts-select", args=[self.account.pk]), {"account_type": "demo"}, format="json")
