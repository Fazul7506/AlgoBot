...
class DerivAdapter(BrokerAdapter):
    ...
    async def _stream(self, subscriptions, callback=None, authenticated=False, stream_name="broker"):
        ...
        while True:
            ...
            try:
                endpoint = await asyncio.to_thread(self._authenticated_ws_url) if authenticated else self.endpoint
                ws = await websockets.connect(endpoint, open_timeout=self.timeout, close_timeout=5, ping_interval=20, ping_timeout=10, max_size=2**20)
    ...
    async def connect(self):
        start = time.perf_counter(); account = await self.authenticate(); latency = (time.perf_counter() - start) * 1000; self._last_connection_latency = latency
    ...
    async def get_chart_history(self, symbol, mode="ticks", count=120, granularity=None):
        ...
        if mode == "candles":
            ...
            payload = {"ticks_history": symbol, "end": "latest", "count": count, "style": "candles", "granularity": int(granularity)}; candles = (await self._request(payload)).get("candles", [])
            return {"mode": "candles", "symbol": symbol, "granularity": int(granularity), "items": candles}
        payload = {"ticks_history": symbol, "end": "latest", "count": count, "style": "ticks"}; ticks = (await self._request(payload)).get("history", {})
    ...
    async def place_order(self, order):
        ...
        live_contracts = await self.get_trade_capabilities(order.symbol); allowed = {str(item.get("contract_type") or "").upper() for item in live_contracts}
        if requested_contract not in allowed: raise BrokerOrderError(f"Deriv does not currently offer {requested_contract} for {order.symbol}")
        expected_direction = {"ACCU": "BUY", "MULTUP": "BUY", "MULTDOWN": "SELL", "CALL": "BUY", "PUT": "SELL", "RISE": "BUY", "FALL": "SELL"}.get(requested_contract)
