...
class BrokerAccount(models.Model):
    ...
    def set_refresh_token(self,token): self.refresh_token=CredentialEncryptionService().encrypt(token or '')
    def get_refresh_token(self): return CredentialEncryptionService().decrypt(self.refresh_token) or '' if self.refresh_token else ''
    @property
    def is_token_expired(self): return bool(self.expires_at and self.expires_at<=timezone.now())
    @property
    def credential_status(self):
        auth_type=str((self.broker.metadata or {}).get('auth') or '').lower(); requires_oauth=self.broker.broker_type=='deriv' or auth_type=='oauth'
...
class BrokerConnectionLog(models.Model):
    broker_account=models.ForeignKey(BrokerAccount,on_delete=models.CASCADE,related_name='connection_logs'); status=models.CharField(max_length=50); latency=models.FloatField(null=True,blank=True); event=models.CharField(max_length=120); created_at=models.DateTimeField(auto_now_add=True)
    class Meta: ordering=['-created_at']; indexes=[models.Index(fields=['broker_account','-created_at']),models.Index(fields=['event'])]

class BrokerPermission(models.Model):
    broker=models.ForeignKey(Broker,on_delete=models.CASCADE,related_name='permissions'); permission=models.CharField(max_length=80); enabled=models.BooleanField(default=True)
    class Meta: unique_together=[('broker','permission')]

class Order(models.Model):
    user=models.ForeignKey(settings.AUTH_USER_MODEL,on_delete=models.CASCADE,related_name='broker_orders'); broker=models.ForeignKey(Broker,on_delete=models.PROTECT,related_name='orders'); account=models.ForeignKey(BrokerAccount,on_delete=models.PROTECT,related_name='orders'); strategy=models.CharField(max_length=120,blank=True); symbol=models.CharField(max_length=40,db_index=True); direction=models.CharField(max_length=16,choices=choices(c.DIRECTIONS)); order_type=models.CharField(max_length=32,choices=choices(c.ORDER_TYPES),default='market'); contract_type=models.CharField(max_length=40,choices=choices(c.CONTRACT_TYPES),blank=True); stake=models.DecimalField(max_digits=20,decimal_places=8,default=0); quantity=models.DecimalField(max_digits=20,decimal_places=8,default=0); price=models.DecimalField(max_digits=20,decimal_places=8,null=True,blank=True); status=models.CharField(max_length=32,choices=choices(c.ORDER_STATUSES),default='created',db_index=True); client_order_id=models.CharField(max_length=100,blank=True,db_index=True); broker_order_id=models.CharField(max_length=120,blank=True,db_index=True); routing_context=models.JSONField(default=dict,blank=True); submitted_at=models.DateTimeField(null=True,blank=True); executed_at=models.DateTimeField(null=True,blank=True); created_at=models.DateTimeField(auto_now_add=True); updated_at=models.DateTimeField(auto_now=True)
    class Meta:
        ordering=['-created_at']; indexes=[models.Index(fields=['user','status']),models.Index(fields=['broker','status'],name='brokers_ord_broker__e4e374_idx'),models.Index(fields=['account','status'],name='brokers_ord_acct_stat_idx')]; constraints=[models.UniqueConstraint(fields=['user','account','client_order_id'],condition=~Q(client_order_id=''),name='unique_client_order_id_per_account')]
