...
def ensure_broker_connection_index(apps, schema_editor):
    ...
    schema_editor.add_index(Model, models.Index(fields=['broker_account', 'status'], name=name))
...
def ensure_order_index(apps, schema_editor):
    ...
    schema_editor.add_index(Model, models.Index(fields=['account', 'status'], name=name))
...
def ensure_broker_connection_uniqueness(apps, schema_editor):
    ...
    if name in _existing_db_objects(schema_editor.connection, table):
        return
    Model = apps.get_model('brokers', 'BrokerConnection')
    constraint = models.UniqueConstraint(condition=Q(broker_account__isnull=False), fields=('broker_account',), name=name)
...
def ensure_order_idempotency_constraint(apps, schema_editor):
    ...
    if name in _existing_db_objects(schema_editor.connection, table):
        return
    Model = apps.get_model('brokers', 'Order')
    constraint = models.UniqueConstraint(condition=~Q(client_order_id=''), fields=('user', 'account', 'client_order_id'), name=name)
    schema_editor.add_constraint(Model, constraint)
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.SeparateDatabaseAndState(
            database_operations=[migrations.RunPython(ensure_broker_account_column, migrations.RunPython.noop)],
            state_operations=[
                migrations.AddField(
                    model_name='brokerconnection',
                    name='broker_account',
                    field=models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.CASCADE, related_name='connections', to='brokers.brokeraccount'),
                ),
            ],
        ),
        migrations.SeparateDatabaseAndState(
            database_operations=[migrations.RunPython(ensure_broker_connection_index, migrations.RunPython.noop)],
            state_operations=[
                migrations.AddIndex(
                    model_name='brokerconnection',
        ...
        migrations.SeparateDatabaseAndState(
            database_operations=[migrations.RunPython(ensure_order_index, migrations.RunPython.noop)],
            state_operations=[
                migrations.AddIndex(
                    model_name='order',
        ...
        migrations.SeparateDatabaseAndState(
            database_operations=[migrations.RunPython(ensure_broker_connection_uniqueness, migrations.RunPython.noop)],
            state_operations=[
                migrations.AddConstraint(
                    model_name='brokerconnection',
                    constraint=models.UniqueConstraint(condition=Q(broker_account__isnull=False), fields=('broker_account',), name='unique_broker_connection_per_account'),
                ),
            ],
        ),
        migrations.SeparateDatabaseAndState(
            database_operations=[migrations.RunPython(ensure_order_idempotency_constraint, migrations.RunPython.noop)],
            state_operations=[
                migrations.AddConstraint(
                    model_name='order',
                    constraint=models.UniqueConstraint(condition=~Q(client_order_id=''), fields=('user', 'account', 'client_order_id'), name='unique_client_order_id_per_account'),
                ),
            ],
        ),
    ]
