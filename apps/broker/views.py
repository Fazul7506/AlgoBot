...
def _bridge_existing_multi_broker_account(user):
    ...
    Older OAuth callbacks persisted the live Deriv account in apps.brokers only,
    while this API reads apps.broker. Bridge it on read so an already-authorized
    user does not have to reconnect Deriv.
    """
    try:
        ...
        if not source:
            return
        if not str(source.broker.broker_type).lower().startswith("deriv"):
            return
        ...
        # The status endpoint must remain available even if an optional legacy
        # bridge cannot be reconstructed. The source API response is still valid.
        return
