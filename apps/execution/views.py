...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    def preview(self, request):
        """Return an authoritative pre-trade preview without creating or sending an order."""
        ...
        return response.Response({
            'status':'ready','source':'authoritative_pre_trade_preview','account':{
                'id':account.id,'broker':account.broker.name,'account_id':account.account_id,'environment':environment,
                'supports_live':bool(getattr(account.broker,'supports_live',False))},
            'order':{'symbol':data.get('symbol'),'direction':data.get('direction'),'order_type':data.get('order_type'),'stake':stake_value,'strategy':data.get('strategy','')},
            'market':{'price':float(quote.last_price),'bid':float(quote.bid) if quote.bid is not None else None,'ask':float(quote.ask) if quote.ask is not None else None,'spread':float(quote.spread or 0),'timestamp':quote.timestamp.isoformat()},
            'gates':{'account_connected':True,'environment_verified':True,'live_trading_allowed':environment != 'real' or bool(getattr(__import__('django.conf',fromlist=['settings']).settings,'ALLOW_LIVE_TRADING',False)),'fresh_market_data':True}
        })
