...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    def _environment(account):
        return str((account.credentials or {}).get('account_type') or '').lower().strip() if account else ''

    @staticmethod
    def _safe_client_context(data):
        context = data.get('validation_context') or {}
        return {
            'broker_source': context.get('broker_source') or 'connected_broker',
            'contract_type': context.get('contract_type'),
            'underlying_symbol': context.get('underlying_symbol') or data.get('symbol'),
            'selected_strategy': context.get('selected_strategy') or data.get('strategy') or None,
        }
    ...
    def create(self, request, *args, **kwargs):
        ...
        data['validation_context'] = self._safe_client_context(request.data)
        ...
        if environment == 'real':
            ...
            if not bool(getattr(settings, 'ALLOW_LIVE_TRADING', False)):
                return response.Response({'status': 'rejected', 'code': 'LIVE_TRADING_DISABLED', 'detail': 'Live-money trading is disabled by platform configuration.'}, status=status.HTTP_409_CONFLICT)
            try:
                order = ExecutionEngine().place_consensus_order(request.user, symbol=data.get('symbol'), timeframe='M1', context=None, risk_context={}, **data)
    ...
    def preview(self, request):
        ...
        if account is None or account.user_id != request.user.id:
            return response.Response({'status': 'rejected', 'code': 'BROKER_ACCOUNT_REQUIRED', 'detail': 'Select a connected broker account.'}, status=status.HTTP_409_CONFLICT)
        if not account.is_connection_eligible:
            return response.Response({'status': 'rejected', 'code': 'BROKER_ACCOUNT_NOT_READY', 'detail': 'The selected broker account is not connected or its credentials are not usable.'}, status=status.HTTP_409_CONFLICT)
        environment = self._environment(account)
        if not environment:
            return response.Response({'status': 'rejected', 'code': 'ACCOUNT_ENVIRONMENT_UNVERIFIED', 'detail': 'Broker account environment has not been verified.'}, status=status.HTTP_409_CONFLICT)
        if environment == 'real' and not bool(getattr(account.broker, 'supports_live', False)):
            return response.Response({'status': 'rejected', 'code': 'LIVE_BROKER_UNSUPPORTED', 'detail': 'The selected broker is not live-trading capable.'}, status=status.HTTP_409_CONFLICT)
        if environment == 'real':
            allowed, used, limit = check_live_order(request.user)
            if not allowed:
                return response.Response({'status': 'rejected', 'code': 'LIVE_ORDER_LIMIT_REACHED', 'detail': f'Your {effective_plan(request.user).name} live-trading allowance has been reached for today.', 'plan': effective_plan(request.user).key, 'used': used, 'limit': limit}, status=status.HTTP_429_TOO_MANY_REQUESTS)
        ...
        if environment == 'real':
            try:
                ...
                prediction = PredictionService().predict(data.get('symbol'), 'M1', None)
                ...
                if not approved:
                    return response.Response({'status': 'rejected', 'code': 'AI_CONSENSUS_GATE_REJECTED', 'detail': reason}, status=status.HTTP_409_CONFLICT)
                ...
                return response.Response({'status': 'rejected', 'code': 'AI_ANALYSIS_UNAVAILABLE', 'detail': f'Live execution requires a fresh verified AI decision: {exc}'}, status=status.HTTP_409_CONFLICT)

        stake = data.get('stake')
        try:
            stake_value = float(Decimal(str(stake)))
    ...
    def cancel(self, request, pk=None):
        return response.Response(OrderSerializer(ExecutionEngine().cancel_order(self.get_object())).data)

    @decorators.action(detail=True, methods=['post'])
    def retry(self, request, pk=None):
        ExecutionEngine().retry(self.get_object())
...
class ReconciliationEventViewSet(viewsets.ReadOnlyModelViewSet):
    serializer_class = ReconciliationEventSerializer
    permission_classes = [permissions.IsAuthenticated]
    def get_queryset(self):
