...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    def _ai_context(data):
        return data.get('validation_context') or {}

    @classmethod
    def _require_ai_context(cls, data):
        context = cls._ai_context(data)
        prediction = str(context.get('ai_prediction') or '').upper()
        recommendation = str(context.get('ai_recommendation') or '').upper()
        confidence = context.get('ai_confidence')
        actionable = context.get('ai_actionable')
        if not prediction and not recommendation:
            return False, {'status':'rejected','code':'AI_ANALYSIS_REQUIRED','detail':'Run AI analysis for the selected broker market before placing an order.'}
        if prediction not in {'BUY','SELL','AVOID'} and recommendation not in {'BUY','SELL','AVOID','WAIT'}:
            return False, {'status':'rejected','code':'AI_DECISION_INVALID','detail':'The AI decision is not a supported BUY, SELL or AVOID decision.'}
        if recommendation in {'AVOID','WAIT'} or prediction == 'AVOID' or actionable is False:
            return False, {'status':'rejected','code':'AI_DECISION_NOT_ACTIONABLE','detail':'AI analysis did not approve this market for trading.'}
        try:
            if confidence is not None and float(confidence if float(confidence) <= 1 else float(confidence) / 100) < .65:
                return False, {'status':'rejected','code':'AI_CONFIDENCE_TOO_LOW','detail':'AI confidence is below the 65% execution gate.'}
        except (TypeError, ValueError):
            return False, {'status':'rejected','code':'AI_CONFIDENCE_INVALID','detail':'AI confidence is invalid. Run AI analysis again.'}
        return True, None
    ...
    def preview(self, request):
        ...
        ai_ok, ai_error = self._require_ai_context(data)
        if not ai_ok:
            return response.Response({**ai_error, 'ai_gate': {'required': True, 'approved': False, 'reason': ai_error['detail']}}, status=status.HTTP_409_CONFLICT)
        try:
        ...
        try:
            ...
            timeframe=str(self._ai_context(data).get('timeframe') or 'M1').upper()
            prediction=PredictionService().predict(data.get('symbol'), timeframe, data.get('ai_context') or {})
            approved, reason=ConsensusDecisionGate().validate(prediction, data.get('direction'))
            consensus=(prediction.payload or {}).get('consensus') or {}
            ...
            if not approved:
                return response.Response({'status':'rejected','code':'AI_CONSENSUS_REJECTED','detail':reason,'ai_gate':ai_gate}, status=status.HTTP_409_CONFLICT)
        except PermissionError as exc:
            return response.Response({'status':'rejected','code':'AI_CONSENSUS_REJECTED','detail':str(exc),'ai_gate':{**ai_gate,'reason':str(exc)}}, status=status.HTTP_409_CONFLICT)
        except Exception as exc:
            return response.Response({'status':'rejected','code':'AI_GATE_FAILED','detail':f'AI pre-trade validation failed: {exc}','ai_gate':{**ai_gate,'reason':str(exc)}}, status=status.HTTP_409_CONFLICT)
