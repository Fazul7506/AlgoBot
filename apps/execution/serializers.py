...
class OrderSerializer(serializers.ModelSerializer):
    ...
    def validate_direction(self, value):
        value = str(value).strip().lower(); allowed = {choice[0] for choice in Order.DIRECTION_CHOICES}
        if value not in allowed: raise serializers.ValidationError(f'Unsupported order direction: {value}')
        return value
    def validate_order_type(self, value):
        value = str(value).strip().lower(); allowed = {choice[0] for choice in Order.ORDER_TYPE_CHOICES}
        if value not in allowed: raise serializers.ValidationError(f'Unsupported order type: {value}')
        return value
