...
class ExecutionEngineTests(TestCase):
    def setUp(self):
        ...
        self.broker=Broker.objects.create(name='Test Broker',slug='test')
        self.account=BrokerAccount.objects.create(user=self.user,broker=self.broker,broker_account_id='A1',balance=Decimal('100'),is_connected=True)
