...
class ExecutionEngine:
    def _ai_market_context(self, account, symbol, timeframe='M1'):
        ...
        async def fetch():
            return await asyncio.wait_for(BrokerRegistry().adapter(account.broker, account).get_market_data(symbol), timeout=7.0)
        tick_data = asyncio.run(fetch())
        price = tick_data.get('price', tick_data.get('quote'))
    ...
    def place_consensus_order(self, user, *, symbol, timeframe='M1', context=None, risk_context=None, priority=5, **data):
        ...
        context = context if context else self._ai_market_context(account, symbol, timeframe)
        prediction = PredictionService().predict(symbol, timeframe, context)
    ...
    async def execute(self, order):
        ...
        if validation.get('ai_source') == 'ensemble':
            ...
            if decision not in {'BUY', 'SELL'} or decision != str(order.direction).upper() or confidence < 65.0 or int(consensus.get('models_used', 0) or 0) < 1: raise PermissionError('Ensemble consensus validation failed at execution boundary')
        from apps.risk.engine import RiskEngine
        ...
        ExecutionLogRepository().log(order, 'OrderExecuted', 'success', 'Broker accepted order', (time.perf_counter() - start) * 1000, response)
        return order
