...
class ExecutionEngine:
    ...
    def _assert_authoritative_account(user, account):
        ...
        if not account.is_connection_eligible: raise PermissionError('The selected broker account is not connected or its credentials are not usable')
        return account
    ...
    def _order_persistence_data(data):
        ...
        persisted = dict(data)
        risk_context = persisted.pop('risk_context', None) or {}
        priority = persisted.pop('priority', 5)
        persisted['validation_context'] = {**(persisted.get('validation_context') or {}), 'risk_context': risk_context}
    ...
    def place_order(self, user, **data):
        ...
        order = OrderService().create_order(user, **persisted); OrderValidationService().validate(order)
        from apps.risk.engine import RiskEngine
        RiskEngine().approve_or_raise(order, risk_context); ExecutionQueueService().enqueue(order, priority); return order

    def place_manual_order(self, user, **data):
        """Execute an explicit terminal Buy/Sell command synchronously.

        Manual terminal clicks are user commands, not strategy jobs. They must
        not enter the background execution queue or wait for a worker. The
        broker response is returned directly to the terminal. If the broker
        connection fails after the order is marked sent, the caller can safely
        reconcile the authoritative broker state without an automatic retry.
        """
        self._assert_authoritative_account(user, data.get('broker_account'))
        persisted, _priority, risk_context = self._order_persistence_data(data)
        persisted['validation_context'] = {**persisted.get('validation_context', {}), 'execution_mode': 'manual_command'}
        order = OrderService().create_order(user, **persisted)
        OrderValidationService().validate(order)
        from apps.risk.engine import RiskEngine
        RiskEngine().approve_or_raise(order, risk_context)
        return asyncio.run(self.execute(order))

    def place_consensus_order(self, user, *, symbol, timeframe='M1', context=None, risk_context=None, priority=5, **data):
        """Strategy-only autonomous execution path.

        This path is deliberately separate from the terminal/manual order
        command path. Existing strategy, AI-consensus and risk gates remain
        authoritative before an order can enter the execution queue.
        """
        ...
        data['validation_context'] = {**(data.get('validation_context') or {}), 'ai_consensus': consensus, 'ai_prediction_id': prediction.pk, 'ai_recommendation_id': recommendation.pk, 'ai_source': 'ensemble', 'execution_mode': 'strategy_auto'}
        effective_risk = {**(risk_context or data.pop('risk_context', {}) or {}), 'ai_consensus_confidence': consensus.get('confidence', prediction.confidence), 'ai_consensus_decision': direction}
        data['validation_context'] = {**data['validation_context'], 'risk_context': effective_risk}
        order = OrderService().create_order(user, **data); OrderValidationService().validate(order)
        from apps.risk.engine import RiskEngine
        RiskEngine().approve_or_raise(order, effective_risk); ExecutionQueueService().enqueue(order, priority); return order
