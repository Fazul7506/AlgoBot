...
class ExecutionEngine:
    def place_order(self, user, **data):
        ...
        RiskEngine().approve_or_raise(order, data.get('risk_context') or {})
        ExecutionQueueService().enqueue(order, data.get('priority', 5))
        return order
    ...
    def place_consensus_order(self, user, *, symbol, timeframe='M1', context=None, risk_context=None, priority=5, **data):
        ...
        data['validation_context'] = {
            **(data.get('validation_context') or {}),
            'ai_consensus': consensus,
            'ai_prediction_id': prediction.pk,
            'ai_recommendation_id': recommendation.pk,
            'ai_source': 'ensemble',
        }
        data['risk_context'] = {
            **(risk_context or data.get('risk_context') or {}),
            'ai_consensus_confidence': consensus.get('confidence', prediction.confidence),
            'ai_consensus_decision': direction,
        }
        order = OrderService().create_order(user, **data)
        OrderValidationService().validate(order)
        from apps.risk.engine import RiskEngine
        RiskEngine().approve_or_raise(order, data.get('risk_context') or {})
        ExecutionQueueService().enqueue(order, priority)
        return order

    def cancel_order(self, order):
        return OrderService().cancel(order)

    def modify_order(self, order, **changes):
        return OrderService().modify(order, **changes)

    async def execute(self, order):
        """Final execution boundary; re-checks safety immediately before broker submission."""
        # Never submit an order twice after the broker has already accepted/executed it.
        if order.status in {c.ORDER_STATUS_ACCEPTED, c.ORDER_STATUS_EXECUTED}:
            return order
        if order.status in {c.ORDER_STATUS_CANCELLED, c.ORDER_STATUS_FAILED}:
            raise PermissionError(f'Cannot execute order in {order.status} state')

        validation = getattr(order, 'validation_context', {}) or {}
        consensus = validation.get('ai_consensus') or {}
        if validation.get('ai_source') == 'ensemble':
            decision = str(consensus.get('decision', '')).upper()
        ...
        # The order may have waited in the queue. Re-run the risk gate immediately
        # before the broker call so a changed account/risk state cannot bypass safety.
        from apps.risk.engine import RiskEngine
        ...
        order.broker_response = response or {}
        order.broker_reference = str(
            (response or {}).get('broker_order_id')
            or (response or {}).get('contract_id')
            or (response or {}).get('order_id', '')
        )
        order.status = c.ORDER_STATUS_EXECUTED
        order.save(update_fields=['broker_response', 'broker_reference', 'status', 'updated_at'])
    ...
    def retry(self, order):
        return ExecutionQueueService().enqueue(order, queue_type='retry')

    def close_position(self, position, exit_price):
        return PositionService().close_position(position, exit_price)

    async def synchronize(self, broker_account):
        return await TradeSynchronizationService().synchronize(broker_account)

    def rollback(self, order, message='Execution rolled back'):
        order.status = c.ORDER_STATUS_FAILED
        order.save(update_fields=['status', 'updated_at'])
        ExecutionLogRepository().log(order, 'ExecutionFailed', 'failed', message)
        return order
