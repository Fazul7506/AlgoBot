...
class ExecutionEngine:
    def place_order(self, user, **data):
        ...
        AI/ensemble-gated execution should use place_consensus_order().
        """
    ...
    def place_consensus_order(self, user, *, symbol, timeframe='M1', context=None, risk_context=None, priority=5, **data):
        """Create an order only when the ensemble produces an actionable consensus.

        Flow: market context -> PredictionService -> ensemble consensus ->
        RecommendationService -> consensus gate -> RiskEngine -> queue.
        The final order direction is always taken from the ensemble, never from
        an independently supplied direction.
        """
        ...
        direction=(prediction.payload or {}).get('consensus',{}).get('decision', prediction.prediction)
        if direction not in {'BUY','SELL'}:
            raise PermissionError('Ensemble returned a non-trade decision')
        data['direction']=direction
        data['validation_context']={
            **(data.get('validation_context') or {}),
            'ai_consensus': prediction.payload.get('consensus',{}),
        ...
        data['risk_context']={
            **(risk_context or data.get('risk_context') or {}),
            'ai_consensus_confidence': prediction.payload.get('consensus',{}).get('confidence', prediction.confidence),
    ...
    async def execute(self, order):
        ...
        broker_order = SimpleNamespace(
            ...
            routing_context=getattr(order, 'validation_context', {}) or {},
        )
