...
class Order(models.Model):
    ...
    STATUS_CHOICES = [
        (v, v.replace('_', ' ').title())
        for v in [
            c.ORDER_STATUS_DRAFT,
            c.ORDER_STATUS_VALIDATED,
            c.ORDER_STATUS_QUEUED,
            c.ORDER_STATUS_SENT,
            c.ORDER_STATUS_ACCEPTED,
            c.ORDER_STATUS_EXECUTED,
            c.ORDER_STATUS_ARCHIVED,
            c.ORDER_STATUS_FAILED,
            c.ORDER_STATUS_CANCELLED,
        ]
    ]
    ...
    broker_account = models.ForeignKey(
        'brokers.BrokerAccount', on_delete=models.PROTECT, related_name='execution_orders'
    )
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=['broker_account', 'symbol']),
        ]
        constraints = [
            models.UniqueConstraint(
                fields=['user', 'client_request_id'],
                condition=~models.Q(client_request_id=''),
                name='unique_execution_client_request',
            )
        ]

    def __str__(self):
        return f'{self.symbol} {self.direction} {self.stake}'
...
class ExecutionQueue(models.Model):
    ...
    def mark_retry(self, delay_seconds=30):
        self.status = c.QUEUE_STATUS_RETRY
        self.attempts += 1
        self.next_retry = timezone.now() + timezone.timedelta(seconds=delay_seconds)
        self.save(update_fields=['status', 'attempts', 'next_retry', 'updated_at'])
        return self
