...
def _analytics_context(user):
    ...
    return {
        ...
        "equity_curve": list(snapshots.values("created_at", "equity", "balance", "daily_pnl", "risk_utilization")),
...
def analysis_data(request):
    ...
    candles = list(Candle.objects.filter(symbol=market, timeframe=timeframe).order_by("-epoch")[:limit])
    candles.reverse()
