...
class ContractServiceTests(TestCase):
    def test_purchase_contract(self):
        user=get_user_model().objects.create_user('c@example.com','c@example.com','pass'); broker=Broker.objects.create(name='CB',broker_type='deriv'); acct=BrokerAccount.objects.create(user=user,broker=broker,broker_account_id='C')
