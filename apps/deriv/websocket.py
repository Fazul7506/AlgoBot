...
class DerivWebSocketEngine:
    ...
    async def send(self, payload: dict[str, Any]) -> str:
        req_id = str(payload.get("req_id") or uuid.uuid4().int % 2147483647)
    ...
    async def heartbeat(self) -> None:
        while self.connected:
            try:
                ...
                logger.exception("Deriv heartbeat failed")
                await self.reconnect()
            await asyncio.sleep(20)

    async def subscribe(self, symbol: str) -> str:
        req_id = await self.send({"ticks": symbol, "subscribe": 1})
    ...
    async def unsubscribe(self, symbol: str) -> None:
        ...
        if req_id:
            await self.send({"forget": req_id})

    async def reconnect(self) -> None:
        self.connected = False
        await asyncio.sleep(self.reconnect_delay)
        await self.connect()
        for payload in list(self.retry_queue):
...
class DerivSubscriptionManager:
    ...
    async def unsubscribe(self, symbol: str):
        await self.engine.unsubscribe(symbol)
        self.symbols = [s for s in self.symbols if s != symbol]

    async def subscribe_all(self):
        for symbol in self.symbols:
            await self.engine.subscribe(symbol)
