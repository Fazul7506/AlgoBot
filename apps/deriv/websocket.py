"""Enterprise websocket engine for Deriv with reconnects, heartbeats and subscriptions."""
...
class DerivWebSocketEngine:
    def __init__(self, app_id: str | None = None, uri: str = DERIV_WS_URL, reconnect_delay: float = 3.0):
        self.app_id = app_id or getattr(settings, "DERIV_APP_ID", "")
        self.uri = f"{uri}?app_id={self.app_id}" if "app_id=" not in uri else uri
        self.websocket = None; self.connected = False; self.subscriptions = {}; self.handlers: list[Handler] = []
        self.retry_queue = deque(); self.seen: set[str] = set(); self.latency_ms: float | None = None; self.reconnect_delay = reconnect_delay
    def add_handler(self, handler: Handler) -> None: self.handlers.append(handler)
    async def connect(self) -> None:
        self.websocket = await websockets.connect(self.uri, ping_interval=None); self.connected = True; asyncio.create_task(self._listen()); asyncio.create_task(self.heartbeat())
    ...
    async def authorize(self, token: str) -> str: return await self.send({"authorize": token})
    ...
    async def heartbeat(self) -> None:
        while self.connected:
            try: await self.ping()
            except Exception: logger.exception("Deriv heartbeat failed"); await self.reconnect()
...
class DerivSubscriptionManager:
    ...
    async def subscribe(self, symbol: str): self.symbols.append(symbol) if symbol not in self.symbols else None; return await self.engine.subscribe(symbol)
    async def unsubscribe(self, symbol: str): await self.engine.unsubscribe(symbol); self.symbols = [s for s in self.symbols if s != symbol]
