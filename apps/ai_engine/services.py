...
class ConfidenceCalibrationService:
    def calibrate(self,probability:float,risk_score:float=0)->dict:
        score=max(0,min(100,probability*100-(risk_score*20))); label=next(label for threshold,label in reversed(CONFIDENCE_LABELS) if score>=threshold); return {'score':round(score,2),'label':label}
...
class FeatureEngineeringService:
    ...
    def build_features(self,symbol:str,timeframe:str,context:dict[str,Any]|None=None)->dict[str,Any]:
        c=context or {}; md=c.get('market_data',{}); ind=c.get('indicators',{}); sm=c.get('smart_money',{}); risk=c.get('risk',{}); strat=c.get('strategy',{}); o,h,l,cl=map(lambda k:_num(md.get(k)),('open','high','low','close')); volatility=_num(md.get('volatility'),abs(h-l)/(cl or 1)); velocity=_num(md.get('price_velocity'),cl-o)
        ...
        else: features.update({name:0.0 for name in FEATURE_NAMES})
        return features
...
class EnsembleService:
    def combine(self,predictions:Iterable[dict],method='weighted_average'):
        ...
        weights=[max(0.,_num(p.get('weight',1.),1.)) for p in ps]; total=sum(weights) or 1.; scores={'BUY':0.,'SELL':0.,'AVOID':0.}
        ...
        if agreement<.60:decision='AVOID'
...
class ConsensusDecisionGate:
    ...
    def validate(self,prediction,intended_direction=None):
        payload=prediction.payload or {}; consensus=payload.get('consensus') or {}; decision=_decision(consensus.get('decision',prediction.prediction)); confidence=float(consensus.get('confidence',prediction.confidence) or 0); models=int(consensus.get('models_used',0) or 0); pa=payload.get('price_action') or {}; confluence=_num(pa.get('confluence_score')); chop=_num(pa.get('chop_score'))
        ...
        if confluence<.50:return False,'Price-action confluence below 0.50 gate'
        if chop>.80:return False,'Market is too choppy for the price-action gate'
...
class TrainingService:
    def train(self,model=None,mode='manual',symbol=None,timeframe='M1',min_accuracy=.52):
        from .training import MarketModelTrainer
        if symbol:
            MarketModelTrainer().train_symbol(symbol,timeframe,min_accuracy); return TrainingJob.objects.filter(metrics__symbol=symbol).order_by('-started_at').first()
        results=MarketModelTrainer().train_active_symbols(timeframe,min_accuracy); return TrainingJob.objects.create(status='completed',started_at=timezone.now(),completed_at=timezone.now(),metrics={'mode':mode,'results':results,'feature_set':list(FEATURE_NAMES)})
class AIRiskAdvisor:
