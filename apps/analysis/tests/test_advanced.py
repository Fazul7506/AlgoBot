...
class AdvancedAnalysisTests(SimpleTestCase):
    def candles(self, n=320):
        ...
        for i in range(n):
            ...
            if 180 <= i < 200:
                base -= (i - 180) * 0.12
            rows.append({"epoch": i, "open": base, "high": base + 0.5, "low": base - 0.35, "close": base + 0.25, "volume": 1})
        return rows
    ...
    def test_advanced_tools_are_returned(self):
        result = analyze_candles(self.candles(), "R_100", "M1")
        for key in ("market_structure", "events", "fair_value_gaps", "liquidity_sweeps", "supply_demand", "candlestick_patterns", "fibonacci", "volatility_regime"):
