...
def structure_events(rows):
    ...
    if len(highs) >= 2:
        ...
        events.append({"type": "HH" if last["price"] > prev["price"] else "LH", "index": last["index"], "price": last["price"]})
    if len(lows) >= 2:
        prev, last = lows[-2], lows[-1]
        events.append({"type": "HL" if last["price"] > prev["price"] else "LL", "index": last["index"], "price": last["price"]})
    if not rows:
        return events
...
def analyze_candles(candles, symbol="", timeframe=""):
    ...
    events, fvgs = structure_events(rows), fair_value_gaps(rows)
    sweeps, zones, patterns = liquidity_sweeps(rows), supply_demand(rows), candlestick_patterns(rows)
    score, factors = 50.0, []
    if e9 is not None and e21 is not None:
        score += 10 if e9 > e21 else -10; factors.append("EMA 9/21 trend")
    if s50 is not None:
        score += 8 if price > s50 else -8; factors.append("SMA 50 location")
    if s200 is not None:
        score += 8 if price > s200 else -8; factors.append("SMA 200 regime")
    if r is not None:
        if r < 30: score += 8; factors.append("RSI oversold")
        elif r > 70: score -= 8; factors.append("RSI overbought")
    if m and m["histogram"] is not None:
        score += 7 if m["histogram"] > 0 else -7; factors.append("MACD momentum")
    if structure["trend"] == "Bullish structure": score += 7; factors.append("Bullish market structure")
    elif structure["trend"] == "Bearish structure": score -= 7; factors.append("Bearish market structure")
    if patterns and patterns[-1]["bias"] != "neutral":
        score += 4 if patterns[-1]["bias"] == "bullish" else -4; factors.append(patterns[-1]["type"].replace("_", " ").title())
    if sweeps:
        score += 4 if sweeps[-1]["type"] == "sell_side_sweep" else -4; factors.append(sweeps[-1]["type"].replace("_", " ").title())
