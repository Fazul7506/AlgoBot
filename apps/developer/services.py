...
class APIKeyService:
    ...
    def rotate(self, api_key):
        raw_secret = secrets.token_urlsafe(32); now = django_timezone.now()
        api_key.previous_secret = api_key.secret; api_key.previous_secret_expires_at = now + timedelta(minutes=5)
...
class APIGatewayService:
    ...
    def check_rate_limit(self, identity, path):
        limit = int(getattr(settings, "DEVELOPER_API_RATE_LIMIT", 60)); window = int(getattr(settings, "DEVELOPER_API_RATE_WINDOW", 60))
        key = f"developer:rate:{identity}:{path}"; count = cache.get(key, 0)
        if count >= limit:
            RateLimitEvent.objects.create(identity=str(identity), path=path); return False, 0
        cache.set(key, count + 1, window); return True, max(0, limit - count - 1)
    def record_usage(self, request, status_code, started, api_key=None):
        elapsed = (time.monotonic() - started) * 1000; user = getattr(request, "user", None)
        ...
        if api_key: APIKey.objects.filter(pk=api_key.pk).update(last_used=django_timezone.now())
...
class WebhookService:
    ...
    def validate_url(url):
        ...
        try: addresses = {info[4][0] for info in socket.getaddrinfo(host, parsed.port or (443 if parsed.scheme == "https" else 80), type=socket.SOCK_STREAM)}
        except socket.gaierror as exc: raise ValueError("Webhook hostname could not be resolved") from exc
        for address in addresses:
            if not ipaddress.ip_address(address).is_global: raise ValueError("Webhook destination must resolve to a public IP address")
        return parsed
    ...
    def deliver(self, webhook, event, payload=None, timeout=5):
        ...
        if event not in self.EVENT_NAMES: return ServiceResult("skipped", {"reason": "event_unsupported"})
        if webhook.events and event not in webhook.events: return ServiceResult("skipped", {"reason": "event_not_subscribed"})
        try: self.validate_url(webhook.url)
        except ValueError as exc: return ServiceResult("failed", {"reason": "invalid_destination", "error": str(exc)})
        ...
        try:
            ...
            delivery.status = "failed"; delivery.last_error = str(exc); delivery.next_attempt_at = django_timezone.now() + timedelta(minutes=min(60, 2 ** min(delivery.attempts, 5))); delivery.save()
...
class SDKService:
    ...
    def generate(self, language, version="latest"):
        if language not in self.languages: raise ValueError("Unsupported SDK language")
        return ServiceResult("scheduled", {"language": language, "version": version})


class AnalyticsService:
    def aggregate(self, user=None):
        start = django_timezone.now().replace(hour=0, minute=0, second=0, microsecond=0); usage = APIUsageEvent.objects.filter(created_at__gte=start); limits = RateLimitEvent.objects.filter(created_at__gte=start)
        if user is not None: usage = usage.filter(user=user); limits = limits.filter(api_key__user=user)
        latencies = sorted(float(value) for value in usage.values_list("latency_ms", flat=True)); p95 = 0
        if latencies: p95 = round(latencies[max(0, min(len(latencies) - 1, int(0.95 * len(latencies)) - 1))], 2)
...
class DocumentationService:
    def publish(self):
        ...
        return ServiceResult("published", contract)
