...
class PositionServiceTests(TestCase):
    def test_open_and_close_position(self):
        user=get_user_model().objects.create_user('p@example.com','p@example.com','pass'); broker=Broker.objects.create(name='B',broker_type='deriv'); acct=BrokerAccount.objects.create(user=user,broker=broker,broker_account_id='P')
