"""Canonical URL configuration for the AlgoBot Django project."""
...
from core.views import (
    ...
    backtesting_page, predictions_page, settings_page, profile_page,
    terms_page, privacy_page, forgot_password_page, reset_password_page,
...
def billing_success_dispatch(request):
    has_provider_return = bool(
        request.GET.get("reference") or request.GET.get("tracking_id")
        or request.GET.get("OrderTrackingId") or request.GET.get("OrderMerchantReference")
    )
...
def billing_cancel_dispatch(request):
    has_provider_return = bool(
        request.GET.get("provider") or request.GET.get("reference")
        or request.GET.get("tracking_id") or request.GET.get("OrderTrackingId")
    )
    if not request.user.is_authenticated and not has_provider_return:
        return redirect(f"/login/?next={request.path}")
...
urlpatterns = [
    ...
    path("performance/", login_required(performance_center), name="performance_page"),
    path("settings/", login_required(settings_page), name="settings_page"),
    path("profile/", login_required(profile_page), name="profile_page"),
    path("orders/", login_required(orders_page), name="orders_page"),
    ...
    path("developer/", login_required(developer_views.dashboard), name="developer_page"),
    path("developer/keys/", login_required(lambda request: render(request, "developer/api_keys.html")), name="developer_keys_page"),
    path("smart-money/", include("apps.smart_money.page_urls")),
    path("callback/", callback, name="callback"),
    path("billing/plans/", billing_plans, name="billing_plans"),
    path("billing/status/", billing_status, name="billing_status"),
    path("billing/entitlements/", billing_entitlements, name="billing_entitlements"),
    ...
    path("licensing/", licensing_page, name="licensing_page"),
    path("contact/", contact_page, name="contact_page"),
    path("about/", about_page, name="about_page"),
    path("data-deletion/", lambda request: render(request, "core/data_deletion.html"), name="data_deletion_page"),
    ...
    path("api/", include("apps.market_data.urls", namespace="market_data_api")),
    path("api/", include("apps.ai_engine.urls")),
    path("api/", include("apps.backtesting.urls")),
    path("api/", include("apps.portfolio.urls")),
    path("api/developer/", include("apps.developer.urls", namespace="developer_api")),
