"""Ensemble predictor for validated market models."""
import os
import numpy as np
...
class EnsemblePredictor:
    ...
    def predict(self, X):
        if not self.models:
            return {"direction": "NO_MODELS", "probability": 0.0, "confidence": 0.0, "models_used": 0, "model_types": []}
        ...
        for model_type, model in self.models.items():
            try:
                if model_type == "lstm":
                    ...
                    probs.append(float(model.predict_proba(X)[-1, 1]))
                else:
                    probs.append(float(model.predict(X)[-1]))
                models_used.append(model_type)
            except Exception:
                continue

        if not probs:
            return {"direction": "ERROR", "probability": 0.0, "confidence": 0.0, "models_used": 0, "model_types": []}

        avg_prob = float(np.mean(probs))
        std_prob = float(np.std(probs))
        agreement_bonus = max(0.0, 0.2 * (1.0 - std_prob))
        confidence = min(1.0, max(0.0, avg_prob + agreement_bonus))
        return {
